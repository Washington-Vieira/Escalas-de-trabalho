...
# Novo Turno C e seus horários (Turno 3)
turnos_C = {
    "C": "22:30 as 06:30"
}
...
turno_funcionario = st.selectbox('Selecione o Turno', options=list(turnos_A.keys()) + list(turnos_B.keys()) + list(turnos_C.keys()) + ['Lubrificador'])
...
if st.button('Gerar Escala'):
    if empresa_selecionada in empresas:
        ...
        funcionarios_C = [f['nome'] for f in funcionarios if f['turno'] in turnos_C.keys()]
        ...
        escala_C = gerar_escala_turnos(funcionarios_C, data_inicio_str, turnos_C)
        ...
        # Exibir a escala dos Turnos C
        st.subheader(f'Escala Turno C - {empresa_selecionada}')
        df_escala_C = pd.DataFrame(escala_C).T
        df_escala_C.columns = [f'Dia {i+1}' for i in range(num_dias_no_mes)]
        st.table(df_escala_C)
