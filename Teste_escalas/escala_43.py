...
if empresa_selecionada and empresas[empresa_selecionada]['folguistas']:
    ...
    if st.button('Editar Escala de Folguistas'):
        ...
        df_folguistas_editado = st.data_editor(df_folguistas)
        if st.button('Salvar Escala de Folguistas'):
            empresas[empresa_selecionada]['folguistas_escala'] = df_folguistas_editado.to_dict(orient='records')
            salvar_empresas(empresas)
            st.success('Escala de folguistas salva com sucesso!')
    else:
        st.write(df_folguistas)
else:
    st.warning('Selecione uma empresa com folguistas cadastrados para exibir a tabela.')

# Exibir a escala de folguistas salva
if 'folguistas_escala' in empresas[empresa_selecionada]:
