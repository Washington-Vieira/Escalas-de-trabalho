...
def identificar_dias_folga(escala_por_funcao, num_dias_no_mes):
    dias_folga = {f'Dia {i+1}': [] for i in range(num_dias_no_mes)}

    for funcao, escala in escala_por_funcao.items():
        if funcao in ["Caixa", "Frentista"]:
            for nome, dias in escala.items():
                for i, dia in enumerate(dias):
                    if "Folga" in dia:
                        dias_folga[f'Dia {i+1}'].append(nome)
...
if empresa_selecionada and empresas[empresa_selecionada]['funcionarios']:
    ...
    if lista_dataframes:
        ...
        dias_folga = identificar_dias_folga(escala_por_funcao, num_dias_no_mes)
