...
if empresa_selecionada and st.session_state.empresas[empresa_selecionada].funcionarios:
    ...
    if lista_dataframes:
        ...
        st.dataframe(df_final, hide_index=True)  # Adicione hide_index=True aqui
        
else:
    st.warning('Selecione uma empresa com funcionários cadastrados para gerar a escala.')

# Modificar a seção de exibição e edição da escala de folguistas
...
if empresa_selecionada:
    ...
    df_folguistas_editado = st.data_editor(
        ...
        num_rows="fixed",
        hide_index=True  # Adicione esta linha
    )
