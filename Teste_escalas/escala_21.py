...
horario_turno = st.text_input('Horário do Turno', placeholder='Exemplo: 08:00 as 12:00 / 13:20 as 18:00')
...
if st.button('Gerar Escala'):
    if empresa_selecionada in empresas:
        ...
        # Criar dicionários de horários personalizados
        horarios_lubrificador = {k: turnos_lubrificador[k] for k in turnos_lubrificador}
        horarios_A = {f['turno']: f['horario'] for f in funcionarios if f['turno'] in turnos_A.keys()}
        horarios_B = {f['turno']: f['horario'] for f in funcionarios if f['turno'] in turnos_B.keys()}

        # Gerar as escalas
        escala_lubrificador = gerar_escala_turnos(funcionarios_lubrificador, data_inicio_str, horarios_lubrificador)
        escala_A = gerar_escala_turnos(funcionarios_A, data_inicio_str, horarios_A)
        escala_B = gerar_escala_turnos(funcionarios_B, data_inicio_str, horarios_B)
        ...
        colunas = [(datetime.strptime(data_inicio_str, '%Y-%m-%d') + timedelta(days=i)).strftime('%d/%m/%Y') for i in range(num_dias_no_mes)]

        # Exibir a escala do Turno Lubrificador
        st.subheader(f'Escala Turno Lubrificador - {empresa_selecionada}')
