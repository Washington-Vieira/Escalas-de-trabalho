...
def gerar_escala_turnos_por_funcao(funcionarios_por_funcao, data_inicio, turnos_horarios, dias_trabalho=5, dias_folga=1):
    escala_final = {}
    data_atual = datetime.strptime(data_inicio, '%Y-%m-%d')
    ...
    for funcao, funcionarios in funcionarios_por_funcao.items():
        escala = {nome: [] for nome in funcionarios}
        ...
        for i, nome in enumerate(funcionarios):
            ...
            for dia in range(num_dias_no_mes):
                ...
                if data_turno == domingo_folga:
                    ...
                    escala[nome].append(f"Turno {turno_atual}: {horario_atual}")
                else:
    ...
    for turno in ['Turno 1', 'Turno 2', 'Turno 3']:
        ...
        if empresa_selecionada in empresas and turno in empresas[empresa_selecionada]['funcionarios']:
            ...
            for func in funcionarios_turno:
                ...
                funcionarios_por_funcao[funcao].append(nome)

            # Gerar escala para os funcionários agrupados por função
            turnos = turnos_funcionarios[turno]
            escala_por_funcao = gerar_escala_turnos_por_funcao(funcionarios_por_funcao, data_inicio_str, turnos)
            ...
            # Exibir a escala como planilha
            st.dataframe(df_escala)

else:
    st.warning('Cadastre funcionários para gerar a escala!')
