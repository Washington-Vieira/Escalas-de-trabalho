...
def identificar_dias_folga(escala_final, num_dias_no_mes):
    ...
    # Preencher listas com None para garantir que todas tenham o mesmo comprimento
    max_length = max(len(v) for v in dias_folga.values())
    for dia in dias_folga:
        while len(dias_folga[dia]) < max_length:
            dias_folga[dia].append(None)

    # Transformar o dicionário em um DataFrame
    df_dias_folga = pd.DataFrame(dias_folga)
    return df_dias_folga
